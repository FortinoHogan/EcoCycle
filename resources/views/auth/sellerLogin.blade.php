...
@section('konten')
    <div class="flex items-center justify-center mt-20 ">
        <div class="flex flex-col justify-center max-w-lg border bg-[#3C552D] border-[#3C552D] p-10 rounded-md">
            ...
            <form action="{{ route('login_seller.post') }}" class="pt-8" method="POST">
                @csrf
                @method('POST')
    ...
    <div class="flex flex-col items-center pt-6 mb-20">
        <a href="{{ route('buyerLogin.view') }}" class="text-xl text-[#EB5B00] font-semibold mb-4 hover:text-gray-400 transition-all duration-500">Login to your personal Buyer account</a>
        <span class="text-[#3C552D] text-lg font-medium">Doesn't have an account? <a href="{{ route('sellerRegister.view') }}"
                class=" hover:text-gray-400 transition-all duration-500">Create a Seller Account</a></span>
    </div>
@endsection
