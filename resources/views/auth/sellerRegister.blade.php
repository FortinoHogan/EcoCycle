...
@section('konten')
            ...
            <form class="pt-8" method="POST" action="{{ route('register_seller.post') }}">
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <input type="email" name="floating_email" id="floating_email"
                        ...
                        placeholder=" " value="{{ old('floating_email') }}" required />
                ...
                <div class="relative z-0 w-full mb-5 group mt-5">
                    <input type="text" name="floating_storeName" id="floating_storeName"
                        class="block py-2.5 px-0 w-full text-sm text-white bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-accent focus:outline-none focus:ring-0 focus:border-accent peer"
                        placeholder="" value="{{ old('floating_storeName') }}" required />
                    <label for="floating_storeName"
                        class="peer-focus:font-medium absolute text-sm text-gray-200 dark:text-accent duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-accent peer-focus:dark:text-accent peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Store
                        Name</label>
                    @if ($errors->has('floating_storeName'))
                        <div class="text-red-500 text-xs mt-2">{{ $errors->first('floating_storeName') }}</div>
                    @endif
                </div>
                ...
                <div class="relative z-0 w-full mb-5 group mt-5">
                    <input type="tel" pattern="[0-9]{8,15}" name="floating_phone" id="floating_phone"
                        class="block py-2.5 px-0 w-full text-sm text-white bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-accent focus:outline-none focus:ring-0 focus:border-accent peer"
                        placeholder="" value="{{ old('floating_phone') }}" required />
                ...
                <div class="relative z-0 w-full mb-5 group mt-5">
                    <input type="text" name="floating_region" id="floating_region"
                        class="block py-2.5 px-0 w-full text-sm text-white bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-accent focus:outline-none focus:ring-0 focus:border-accent peer"
                        placeholder="" value="{{ old('floating_region') }}" required />
    ...
    <div class="flex flex-col items-center justify-center pt-6 mb-20">
        ...
        <span class="text-[#3C552D] text-lg font-medium">Already have an account? <a
                href="{{ route('sellerLogin.view') }} "
